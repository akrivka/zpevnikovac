<template>
  <div>
    ...
    </div>Z písní v osobní databázi se Ti vytvoří Google Dokument se vzorovými styly.
    <br />Musíš je manuálně změnit (označit řádek a dát u příslušného stylu <i>"Aktualizovat styl [styl], aby odpovídal výběru."</i>)<br>
    Hlavně musíš na <i>Normální text</i> a <i>Nadpis 2</i> dát nějaké tzv. <b>monospace písmo</b> (každé písmeno má stejnou šířku), jinak se ty akordy rozbijou.
    <br />
    ...
    generate() {
      ...
      this.$gapi.getGapiClient().then(gapi => {
        gapi.client.docs.documents
          .create({
            title: store.currentUser.displayName,
            documentStyle: {
              background: {
                color: {}
              },
          ...
          .then(function(response) {
            ...
            for (const songProperty in store.userSongs) {
              if (
                ...
                const song = store.userSongs[songProperty];

                var name = song.songInfo.name;
                var composer = song.songInfo.composer;
                var capo = song.songInfo.capo;
                var content = song.songInfo.content;
                console.log(
